...
.controller('MapController',function($scope, $compile, NgMap)
    ...
    .controller('adminController', ['$scope', '$compile', '$state', '$location', 'recordService', 'updateAdmin', 'updateForms', 'sharedRecord', 'partialsPrefix', function($scope, $compile, $state, location, recordService, updateAdmin, updateForms, sharedRecord, partialsPrefix)
  {
      ...
      var adminViews = {
	  "browse":"admin.browseRecords",
	  "doi":"admin.assignDoiArk"
      };

      var adminViewMode = "admin.browseRecords";

      function setAdminViewMode(value){
	  if(typeof value === 'string')
	      adminViewMode = value;
	  else
	      console.log("Error: tried to set adminViewMode to non-string value");
      }

      $scope.setMode = function(value){
	  setAdminViewMode(value);

	  $state.go(getAdminViewMode());
      ...
      //Load brows admin view on page load
      $state.go(getAdminViewMode());
      ...
      var formType = "admin.browseRecords";
	...
	*/
	...
	recordService.adminGetUsersRecord(recordId)
      ...
      function queryDoiDatabase(queryType){
	  //Contstruct empty record to display text that no results found. Don't want ng-repeat loop in partials/allRecords to try and
	  //print variable that is not defined.
	  var noResultsRecord = {"results":{}};
	  noResultsRecord.results = [{"title":"No results!", "summary":"", "citation":[{"name":""}], "md_pub_date":""}];
